    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                ...
                # For each class that had a margin exceeded, we add a contribution
                # to the weights vector associated to the correct class
                dW[:, y[i]] -= X[i]
...
def svm_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    scores = X.mm(W)
    correct_class_scores = scores.take_along_dim(
        y.reshape((-1, 1)),
        dim=1
    )

    margins = torch.nn.functional.relu(
        scores - correct_class_scores + 1
    )

    # Zeroing out the contributions at the correct class indices
    margins = torch.scatter(
        margins,
        1,
        y.reshape((-1, 1)),
        0.0
    ...
    #############################################################################
    # Loss gradient
    nonzero_margins = (margins > 0).to(W.dtype)

    dW = torch.scatter(
        nonzero_margins,
        1,
        y.reshape((-1, 1)),
        -nonzero_margins.sum(dim=1).reshape((-1, 1))
    )
    dW = X.t().mm(dW) / y.shape[0]
...
def sample_batch(
    ...
    #########################################################################
    samples = torch.randint(0, X.shape[0], (batch_size,))
...
def svm_get_search_params():
    ...
    ###########################################################################
    learning_rates = [2e-3, 5e-3, 8e-3, 1e-2, 4e-2]
    regularization_strengths = [8e-4, 1e-3, 2e-3, 5e-3, 0.01]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    # Train model with given parameterization
    cls.train(
        data_dict["X_train"],
        data_dict["y_train"],
        lr,
    ...
    train_acc = torch.mean(data_dict["y_train"] == y_train_pred, dtype=torch.float32)

    # Compute validation set accuracy
    y_val_pred = cls.predict(data_dict["X_val"])

    val_acc = torch.mean(data_dict["y_val"] == y_val_pred, dtype=torch.float32)
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(n_samples):
        ...
        correct_class_score = scores[y[i]]
        sum_of_exp_scores = torch.exp(scores).sum()

        loss -= correct_class_score

        all_classes_contribution_to_loss = 0.0
        for c in range(W.shape[1]):
            all_classes_contribution_to_loss += torch.exp(scores[c]).item()

            # Analytical gradient
            dW[:, c] += torch.exp(scores[c]) * X[i] / sum_of_exp_scores

            if c == y[i]:
                dW[:, c] -= X[i]

        # print("all_classes_contribution_to_loss", all_classes_contribution_to_loss)
        loss += math.log(all_classes_contribution_to_loss)

    # Adjust to make the overall loss be a mean of the single-sample losses
    loss = loss / X.shape[0]

    # Add regularization loss
    loss += reg * torch.sum(W * W)

    # Regulatization contribution to the gradient
    dW /= n_samples
    dW += 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
...
def softmax_loss_vectorized(
    ...
    # print("X", X.unsqueeze(dim=2).shape)
    # print("softmax_probs", softmax_probs.unsqueeze(dim=1).shape)
    # print(
    #     "X * softmax_probs",
    #     (X.unsqueeze(dim=2) * softmax_probs.unsqueeze(dim=1)).mean(dim=0).shape
    # )
    # sample_losses = -torch.log(normalized_exp_scores)
    sample_losses = -torch.log(
        softmax_probs.take_along_dim(y.view(-1, 1), dim=1)
    )
    ...
    # This first line add the the columns of dW the contributions of each sample
    # to their INCORRECT classes
    dW = (X.unsqueeze(dim=2) * softmax_probs.unsqueeze(dim=1)).sum(dim=0)

    # This next expression add to dW the contributions of each sample to their
    # CORRECT class. It's a bit hard to understand
    dW.scatter_reduce_(
        dim=1,
        index=y.view(-1, 1).expand(-1, X.shape[-1]).t(),
        src=-X.t(),
        reduce="sum",
        include_self=True
    )
    ...
    # Regulatization contribution to gradient
    dW += 2 * reg * W
    # print(W.shape)
    # print(dW.shape)
    #############################################################################
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [8e-3, 2e-2, 3e-2, 4e-2, 8e-2, 0.1]
    regularization_strengths = [8e-4, 1e-3, 2e-3, 5e-3, 0.01]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
