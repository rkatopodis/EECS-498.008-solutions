...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    # shape (N, C).                                                            #
    ############################################################################
    hidden = torch.clamp(torch.matmul(X, W1) + b1, min=0)
...
def nn_forward_backward(
    ...
    ############################################################################
    # Take max along each row to perform the numerical stability trick
    max_scores = scores.max(dim=1, keepdim=True)[0]

    normalized_exp_scores = torch.exp(scores - max_scores)
    softmax_scores = normalized_exp_scores / torch.sum(
        normalized_exp_scores,
        dim=1,
        keepdim=True
    )

    correct_class_softmax_score = torch.take_along_dim(
        softmax_scores,
        y.view(-1, 1),
        dim=1
    )

    data_loss = -torch.log(correct_class_softmax_score).mean()
    ...
    dscores = (softmax_scores - torch.nn.functional.one_hot(y)) / len(y)
    dh2 = dscores.sum(dim=0)
    ...
    daffine1 = torch.where(
        h1 != 0,
        dh1,
        0
    )

    grads["b1"] = daffine1.sum(dim=0)
    grads["W1"] = X.t().mm(daffine1)
    grads["W1"] += 2 * reg * W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
    ...
    # classifier.                                                             #
    ###########################################################################
    learning_rates = [0.8, 1.0, 1.2, 1.5, 1.6]
    hidden_sizes = [1_024, 2_048]
    regularization_strengths = [1e-4, 5e-4, 1e-3, 2e-3]
    learning_rate_decays = [1.0, 0.95, 0.98]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    for hyperparams in product(*get_param_set_fn()):
        lr, hidden_size, reg, lr_decay = hyperparams
        ...
        print(
            f"lr: {lr} | hs: {hidden_size} | reg: {reg} | lr_decay: {lr_decay} | val_acc: {val_acc}"
        )
