...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    for i in range(x_train.shape[0]):
        dists[i, :] = (((x_test - x_train[i]) ** 2)
            .view(x_test.shape[0], -1)
            .sum(dim=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    first_term_squared = torch.sum(
        (x_train ** 2).view(x_train.shape[0], 1, -1),
        dim=-1
    )
    middle_term = -2 * torch.mm(
        x_train.view(x_train.shape[0], -1),
        x_test.view(x_test.shape[0], -1).t()
    )
    second_term_squared = torch.sum(
        (x_test ** 2).view(x_test.shape[0], -1),
        dim=-1
    )

    dists = first_term_squared + middle_term + second_term_squared
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    y_pred = torch.mode(votes, dim=0)[0]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        y_test_pred = predict_labels(
            compute_distances_no_loops(self.x_train, x_test),
            self.y_train,
            k
        )
        ######################################################################
...
def knn_cross_validate(
    ...
    k_to_accuracies = {
        k: [
            KnnClassifier(
                torch.cat(
                    [fold for j, fold in enumerate(x_train_folds) if i != j]
                ),
                torch.cat(
                    [fold for j, fold in enumerate(y_train_folds) if i != j]
                )
            ).check_accuracy(x_train_folds[i], y_train_folds[i], k, quiet=True)
