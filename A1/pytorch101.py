...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # This is the first multiple of 10 after start
    first = (floor(start/10) + 1) * 10 if start % 10 else start

    if stop < first:
        x = torch.tensor([])
    else:
        x = torch.arange(start=first, end=stop, step=10)
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    reverse_rows_idx = x.shape[0] - 1 - torch.arange(x.shape[0])
    y = x[reverse_rows_idx, :]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    std = torch.sqrt(torch.sum((x - mu) ** 2, dim=0) / (x.shape[0] - 1))
