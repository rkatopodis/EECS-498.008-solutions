...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        # Flattening all dimensions of input tensor, except batch dimension
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, o_cache = Linear.forward(h, W2, b2)
        ...
        dh, dW2, db2 = Linear.backward(dscores, o_cache)
        _, dW1, db1 = Linear_ReLU.backward(dh, h_cache)

        # Accumulating regularization contribution to loss and gradients
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        layer_output_dims = hidden_dims + [num_classes]

        weights = {
            f"W{i + 1}":  weight_scale * torch.randn(s, dtype=dtype, device=device)

            for i, s in enumerate(zip(layer_input_dims, layer_output_dims))
        }

        biases = {
            f"b{i + 1}": torch.zeros(s, dtype=dtype, device=device)
            
            for i, s in enumerate(layer_output_dims)
        }
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            h, cache = Linear_ReLU.forward(h, W, b)
            hidden_caches.append(cache)

            if self.use_dropout:
                h, do_cache = Dropout.forward(h, self.dropout_param)
                dropout_caches.append(do_cache)

        W, b = self.params[f"W{i + 1}"], self.params[f"b{i + 1}"]

        scores, o_cache = Linear.forward(h, W, b)
        ...
        dh, dW, db = Linear.backward(dscores, o_cache)

        grads[f"W{i}"] = dW
        grads[f"b{i}"] = db

        for i in reversed(range(len(hidden_caches))):
        # for cache in reversed(hidden_caches):
            ...
            dh, dW, db = Linear_ReLU.backward(dh, hidden_caches[i])
...
def get_three_layer_network_params():
    ...
    ###############################################################
    weight_scale = 4e-2 # 5e-3   # Experiment with this!
    learning_rate = 4e-1  # Experiment with this!
    ################################################################
    #                             END OF YOUR CODE                 #
    ################################################################
...
def get_five_layer_network_params():
    ...
    # model achieves 100% training accuracy within 20 epochs.      #
    ################################################################
    weight_scale = 4e-1 # 5e-3   # Experiment with this!
...
def adam(w, dw, config=None):
    ...
    lr, beta1, beta2, eps, m, v, t = (
        config[k] for k in ("learning_rate", "beta1", "beta2", "epsilon", "m", "v", "t")
    )
